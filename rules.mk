...
ifeq ($(CONFIG_UPDATE_TOOLCHAIN), )
  ...
  ifeq ($(ARCH), arm)
    ifeq ($(TARGET_BOARD_PLATFORM), v5)
      CONFIG_TOOLCHAIN_PREFIX=$(ARCH)-linux-$(TARGET_SUFFIX)-
    else
  ...
  ifeq ($(TARGET_CPU_VARIANT),arm926ej-s)
    ...
    ifeq ($(TARGET_BOARD_PLATFORM), v5)
      CONFIG_TOOLCHAIN_ROOT=$(TOPDIR)/prebuilt/gcc/linux-x86/$(ARCH)/toolchain-sun8iw12p1-$(CONFIG_TOOLCHAIN_LIBC)/toolchain
    else
...
ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
  ...
  ifeq ($(TARGET_BOARD_PLATFORM), v5)
    REAL_GNU_TARGET_NAME=$(OPTIMIZE_FOR_CPU)-linux$(if $(TARGET_SUFFIX),-$(TARGET_SUFFIX))
...
ifndef DUMP
  ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
    ...
    TARGET_CROSS:=$(if $(TARGET_CROSS),$(TARGET_CROSS),$(OPTIMIZE_FOR_CPU)-linux$(if $(TARGET_SUFFIX),-$(TARGET_SUFFIX))-)
...
export STAGING_DIR STAGING_DIR_HOST TOOLCHAIN_ROOT_DIR
...
TARGET_Fortran:=$(TARGET_CROSS)gfortran
