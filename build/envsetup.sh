...
PLATFORM_CHOICES=(nuclear astar octopus tulip azalea sitar cello banjo x15 v5)
...
VARIANT_CHOICES=(vecoo dragonboard)
...
function lunch()
{
    ...
    if [ -z "$answer" ]
    then
        selection=azalea_aiworld-vecoo
...
function make_ota_image(){
    ...
    grep -v -e CONFIG_TARGET_ROOTFS_INITRAMFS  $T/target/vecoo/${TARGET_BOARD}/defconfig_ota > .config
    echo 'CONFIG_TARGET_ROOTFS_INITRAMFS=y' >> .config
	echo 'CONFIG_TARGET_AW_OTA_INITRAMFS=y' >> .config
	echo 'CONFIG_TARGET_INITRAMFS_COMPRESSION_XZ=y' >> .config
	cp .config $T/target/vecoo/${TARGET_BOARD}/defconfig
    #refresh_ota_env
    make V=s -j16

	cp $OTA_DIR/.config.old $T/.config
    cp $OTA_DIR/.config.old $T/target/vecoo/${TARGET_BOARD}/defconfig
...
function pack() {
	...
	if [ "x$board_platform" = "xunclear" ]; then
		...
		chip=sun8iw11p1
	elif [ "x$board_platform" = "xv5" ]; then
		chip=sun8iw12p1
...
function ckernel()
{
    T=$(gettop)
    K=linux-3.4
    if [ "$TARGET_PLATFORM" = "tulip" -o "$TARGET_PLATFORM" = "azalea" -o "$TARGET_PLATFORM" = "sitar" -o "$TARGET_PLATFORM" = "cello" -o "$TARGET_PLATFORM" = "x15" ]; then
	    K=linux-3.10
    fi
    if [ "$TARGET_PLATFORM" = "v5" ]; then
	    K=linux-4.4
    fi
    if [ "$T" ]; then
        \cd $(gettop)/lichee/$K/
...
function cgeneric()
{
    T=$(gettop)
    if [ "$T" ]; then
        \cd $(gettop)/target/vecoo/generic
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function cdevice()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/vecoo/${TARGET_BOARD}
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function cconfigs()
{
    T=$(gettop)
    if [ "$T" ]; then
	    \cd $(gettop)/target/vecoo/${TARGET_BOARD}/configs
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
